...
class Main
{
	...
	public function __construct( $addOnSettings=[], $viewObject=null )
	{
			...
			add_filter( 'ihc_payment_gateway_create_payment_object', [ $this, 'createPaymentObject'], 100, 2 );
	...
	public function paymentGatewayBoxStatus( $status=[], $paymentType='' )
	{
			...
			$settings = ihc_return_meta_arr( 'ump_paystack' );
			if ( !empty( $settings['ump_paystack-secret_key'] ) ) {
					$status['settings'] = 'Completed';
			}
			if ( !empty($settings['ump_paystack-enabled']) ){
	...
	{
			$list[ $this->addOnSettings['slug'] ] = esc_html__( 'PayStack', 'ihc' );
			return $list;
	}
	...
	public function createPaymentObject( $object=null, $paymentType='' )
	{
			...
			if ( class_exists( '\UmpPayStackPro\PayStack' ) ){
					// pro version
					return new \UmpPayStackPro\PayStack();
			}
			return new \UmpPayStack\PayStack();
	}
	...
	public function logo( $imageUrl='', $paymentType='' )
	{
			...
			return UMP_PAYSTACK_URL . 'assets/images/paystack.png';
	}
	...
	public function isPaymentAvailable( $status=false, $type='' )
	{
			...
			$settings = ihc_return_meta_arr( $this->addOnSettings['slug'] );
			if ( !empty( $settings['ump_paystack-enabled'] ) && $settings['ump_paystack-secret_key'] ){
					return true;
			}
			return false;
	}
