...
class Utilities
{
	...
	* @var array
    ...
    {
      $string = '<div class="error"><p>' . sprintf( esc_html__( '%s is inactive. The %s plugin must be active for %s to work. Please install & activate %s.','ultimate-membership-pro-paystack' ), self::$addOnName, self::$pluginParentName, self::$addOnName, self::$pluginParentName )  . '</p></div>';
              $allowedHtml = [
                                'div' 		=> [ 'class' => [] ],
                                'p'				=> [],
              ];
    ...
    {
      $string = '<div class="error"><p>' . sprintf( esc_html__( 'In order to use %s you must have %s starting with v.%s.', 'ultimate-membership-pro-paystack' ), self::$addOnName, self::$pluginParentName, self::$parentMinimumVersion )  . '</p></div>';
              $allowedHtml = [
                                'div' 		=> [ 'class' => [] ],
                                'p'				=> [],
              ];
